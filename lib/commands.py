...
payto_syntax = "payto <recipient> <amount> [label]\n<recipient> can be a myriadcoin address or a label"
...
register_command('createmultisig',       2, 2, False, True,  False, 'similar to myriadcoind\'s command')
register_command('createrawtransaction', 2, 2, False, True,  False, 'similar to myriadcoind\'s command')
register_command('deseed',               0, 0, False, True,  False, 'Remove seed from wallet, creating a seedless, watching-only wallet.')
register_command('decoderawtransaction', 1, 1, False, False, False, 'similar to myriadcoind\'s command')
register_command('getprivatekeys',       1, 1, False, True,  True,  'Get the private keys of a given address', 'getprivatekeys <myriadcoin address>')
...
register_command('getpubkeys',           1, 1, False, True,  False, 'Return the public keys for a wallet address', 'getpubkeys <myriadcoin address>')
...
register_command('signrawtransaction',   1, 3, False, True,  True,  'similar to myriadcoind\'s command')
    ...
    def _mktx(self, outputs, fee = None, change_addr = None, domain = None):
        ...
        for to_address, amount in outputs:
            if not is_valid(to_address):
                raise Exception("Invalid Myriadcoin address", to_address)

        if change_addr:
            if not is_valid(change_addr):
                raise Exception("Invalid Myriadcoin address", change_addr)

        if domain is not None:
            for addr in domain:
                if not is_valid(addr):
                    raise Exception("invalid Myriadcoin address", addr)
